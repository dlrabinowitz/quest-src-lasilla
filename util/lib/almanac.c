/* almanac.c
   ...
   The observatory site is Palomar by default (see DEFAULT_OBSCODE).
...
time from SUNSET to EVENING and from MORNING to SUNRISE.
...
/* overkill to put in these preprocessor flags, perhaps. */
		...
		case 'h':    /* signals short argument ... */
			shval = va_arg(ap, int); /* short and int are the same*/
...

{
	...
	if(year < 1993.0 && year >= 1900.) {
		...
		oprntf("etcorr .. year outside expected range\n");
		delt=0.0;
        }                
...
void print_tonight(date,lat,longit,elevsea,elev,horiz,site_name,stdz,
	zone_name,zabr,use_dst,jdb,jde,short_long,almanac)
...

{
	...
	double hatwilight, jdtwilight, jdetw=0.0, jdmtw=0.0;
	double ill_frac;
	short dow; /* day of week */
	float set_to_rise=0.0, twi_to_twi=0.0/*, hrs*/,
	...
	oprntf(" (d.m), elev. %5.0f m\n",elevsea);
        oprntf("#twilight ends when sun above %7.3f deg\n",TWILIGHT_ELEV);
	...
	jdtwilight = jd_sun_alt(TWILIGHT_ELEV,jdtwilight,lat,longit);  /* accurate */
	jdetw = jdtwilight;
	if(jdtwilight > 0.) {
		oprntf("\n#Evening twilight: ");
	...
	jdtwilight = jd_sun_alt(TWILIGHT_ELEV,jdtwilight,lat,longit);
	jdmtw = jdtwilight;
	if(jdtwilight > 0.) {
	...
	oprntf("#The sun is down for %4.1f hr; %4.1f hr from eve->morn %3.0f deg twilight.\n#",
		set_to_rise,twi_to_twi,TWILIGHT_ELEV);
...
/*
     ...
     printf("UT_EVENING ");print_time1(jdetw,1);printf("\n");
     printf("UT_MIDNIGHT ");print_time1(jdmid,1);printf("\n");
     printf("UT_MORNING ");print_time1(jdmtw,1);printf("\n");
     ...
     printf("LST_SUNSET %9.6lf\n",lst(jdsunset,longit));
     printf("LST_EVENING %9.6lf\n",lst(jdetw,longit));
     printf("LST_MIDNIGHT %9.6lf\n",lst(jdmid,longit));
     printf("LST_MORNING %9.6lf\n",lst(jdmtw,longit));
     printf("LST_SUNRISE %9.6lf\n",lst(jdsunrise,longit));
     printf("LST_MOONRISE %9.6lf\n",lst(jdmoonrise,longit));
     printf("LST_MOONSET %9.6lf\n",lst(jdmoonset,longit));
*/
     almanac->lst_sunrise=lst(jdsunrise,longit);
     almanac->lst_sunset=lst(jdsunset,longit);
     almanac->lst_evening=lst(jdetw,longit);
     almanac->lst_morning=lst(jdmtw,longit);
...
/*      printf("geocentric moon: ");
	...
	printf(" %f \n",geodistmoon);  */
	sun_alt = alt;
	if(alt < TWILIGHT_ELEV) oprntf("The sun is down; there is no twilight.\n");
	else {
		if (alt < -(0.83+horiz))
...
/*      oprntf("Raw = %12.4f, Correction: %f\n",jd,tcor);   diagnostic */
	...
	secz = secant_z(altitude);
	   ...
	   sun_alt = altit(decsun,hasun,lat,&az);
	      ...
	      if(sun_alt <= TWILIGHT_ELEV) oprntf(" ngt");
