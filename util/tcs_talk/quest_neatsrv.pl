...
sub check_ccp {
	...
	printf STDERR "RESTARTING CCP\n";
        if ( $FAKE_OBS ) {
	   docmd "/usr/bin/rsh -l quest quest7 start_ops.fake";
        } else {
	   docmd "/usr/bin/rsh -l quest quest7 start_ops";
        }
	sleep(5);
	return checkstatus();
    }
...
sub hinfoSet {
    ...
    if ( $keyword eq 'filtername' ) {
        hinfoCommand("--filternames $value --filternames $value --filternames $value --filternames $value");
    }
    elsif ( $keyword eq 'filterna' ) {
        hinfoCommand("--filternames $value --filternames $value --filternames $value --filternames $value");
    }
    return(0);
}
